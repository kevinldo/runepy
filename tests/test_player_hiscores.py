...
def test_read_recent_player_stat_changes_returns_stored_changes(
    monkeypatch, sample_stat_changes
):
    """Verify recent stat changes use the recent comparison sentinel."""
    ...
    stat_changes = read_player_stat_changes("Zezt", "recent")

    assert isinstance(stat_changes, PlayerStatChanges)
    assert stat_changes == recent_stat_changes
